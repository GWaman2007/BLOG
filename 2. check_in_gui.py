...
class CRM:
    ...
    def add_save(self, save):
        try:
            query = "INSERT INTO check_in (name, address, phone, room_no) VALUES (%s, %s, %s, %s)"
            values = (save.name, save.address, save.mobile_no, save.room_no)
...
class MOTEL_MANGMENT_checkin:
    ...
    def __init__(self):
        ...
        def tor(self):
            ...
            if self.ch == 1:
                self.price = self.price + (30 * self.no_of_days)
                m[0] = 1
            elif self.ch == 2:
                self.price = self.price + (45 * self.no_of_days)
                m[0] = 2
            elif self.ch == 3:
                self.price = self.price + (76 * self.no_of_days)
                m[0] = 3
            elif self.ch == 4:
                self.price = self.price + (29 * self.no_of_days)
                m[0] = 4
        ...
        root.configure(background="#d3d3d3")  # Set the background color to light grey
        root.configure(highlightbackground="#d3d3d3")
        ...
        self.Message1.configure(text='''Motel Managment System''')
